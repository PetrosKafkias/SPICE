...
const LAYERS = [
  { id: 'base', label: 'Base terrain', active: true },
  { id: 'buildings', label: 'Buildings', active: true },
  { id: 'vegetation', label: 'Vegetation', active: true },
  { id: 'proposals', label: 'Design proposals', active: true },
  { id: 'heatmap', label: 'Feedback heatmap', active: false },
];

const SCENARIOS = [
  { id: 'cycle', label: 'Cycle lane', active: true },
  { id: 'garden', label: 'Community garden', active: false },
  { id: 'seating', label: 'Covered seating', active: false },
];
...
export default function SceneEditorPublicPage() {
  ...
  const activeScenario = scenarios.find((scenario) => scenario.active)?.label || 'No scenario selected';
  ...
  const exportConfiguration = () => {
    const payload = {
      ...
      pan,
      firstPerson,
      layers: layers.map(({ id, label, active }) => ({ id, label, active })),
    ...
    toast.success('Scene configuration downloaded');
  };
  ...
  const exportImage = () => {
    ...
    anchor.click();
    toast.success('Scene image downloaded');
  };

  const shareScene = async () => {
    const url = `${window.location.origin}${window.location.pathname}?scenario=${encodeURIComponent(activeScenario)}&zoom=${zoom}`;
    try {
    ...
    <SpicePublicShell variant="public">
      <div style={{ fontFamily: 'Montserrat, sans-serif' }}>
        ...
        >
          <img
            src={sceneImg}
            alt="3D aerial view of Parko Kritis"
          ...
          <div className="absolute top-5 left-5 w-[200px] sm:w-[220px] flex flex-col gap-3">
            <div className="bg-white bg-opacity-95 p-4 shadow-lg">
              <p className="text-[11px] font-semibold text-[#888] uppercase tracking-wide mb-1">Active Pilot</p>
              ...
              <p className={`mt-2 text-[10px] font-bold uppercase tracking-wide ${persistenceStatus === 'error' ? 'text-red-700' : 'text-[#637948]'}`} role="status">
                {persistenceStatus === 'loading' ? 'Loading saved scene' : persistenceStatus === 'saving' ? 'Saving scene' : persistenceStatus === 'error' ? 'Scene save failed' : 'Scene saved'}
              </p>
            </div>

            <div className="bg-white bg-opacity-95 p-4 shadow-lg">
              <p className="text-[12px] font-semibold text-[#444] mb-3 uppercase tracking-wide">Scenarios</p>
              <div className="flex flex-col gap-2">
                {scenarios.map((scenario) => (
                  <label key={scenario.id} className="flex items-center gap-2 cursor-pointer">
                    ...
                    <span className="text-[13px] text-[#444]">{scenario.label}</span>
                  </label>
                ))}
              </div>
            </div>
              ...
              <div className="flex flex-col gap-2">
                {layers.map((layer) => (
                  <label key={layer.id} className="flex items-center gap-2 cursor-pointer">
                    ...
                    <span className="text-[12px] text-[#444]">{layer.label}</span>
                  </label>
                ))}
              </div>
            </div>
          ...
          <div className="absolute top-5 right-5 flex flex-col gap-2">
            {[
              { icon: ZoomIn, label: 'Zoom in', onClick: () => setZoom((value) => Math.min(value + 10, 200)) },
              { icon: ZoomOut, label: 'Zoom out', onClick: () => setZoom((value) => Math.max(value - 10, 60)) },
              { icon: RotateCcw, label: 'Reset view', onClick: resetView, active: false },
              { icon: Move, label: 'Pan', onClick: () => setPanMode((value) => !value), active: panMode },
              { icon: Eye, label: 'First person view', onClick: () => setFirstPerson((value) => !value), active: firstPerson },
              { icon: Maximize2, label: 'Fullscreen', onClick: () => sceneRef.current?.requestFullscreen(), active: false },
              ...
              <button
                ...
                aria-pressed={label === 'Pan' || label === 'First person view' ? active : undefined}
              >
        ...
        <div className="spice-page spice-wide-page grid grid-cols-1 md:grid-cols-3 gap-5">
          <div className="bg-white border-2 border-[#bfc0c5] p-5 shadow-sm">
            <div className="flex items-center gap-2 mb-3">
              ...
              <p className="text-[15px] font-bold text-[#444]">Scene details</p>
            </div>
            <div className="flex flex-col gap-2 text-[13px]">
              <div className="flex justify-between gap-3"><span className="text-[#888]">Pilot</span><span className="font-semibold text-[#444]">Thessaloniki</span></div>
              <div className="flex justify-between gap-3"><span className="text-[#888]">Location</span><span className="font-semibold text-[#444]">Parko Kritis</span></div>
              <div className="flex justify-between gap-3"><span className="text-[#888]">Phase</span><span className="font-semibold text-[#444]">Co-Design (Phase 3)</span></div>
              <div className="flex justify-between gap-3"><span className="text-[#888]">Active scenario</span><span className="font-semibold text-[#ca7428] text-right">{activeScenario}</span></div>
            </div>
          </div>
          ...
          <div className="bg-white border-2 border-[#bfc0c5] p-5 shadow-sm">
            <div className="flex items-center gap-2 mb-3">
              ...
              <p className="text-[15px] font-bold text-[#444]">Environment</p>
            </div>
            <div className="flex flex-col gap-3">
              <div>
                <p className="text-[12px] text-[#888] mb-1">Time of day</p>
                <input type="range" min={0} max={100} value={timeOfDay} onChange={(event) => setTimeOfDay(Number(event.target.value))} className="w-full accent-[#f68b2c]" />
              </div>
              <div>
                <p className="text-[12px] text-[#888] mb-1">Visible layers</p>
                <p className="text-[15px] font-semibold text-[#444]">{activeLayerCount} of {layers.length}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white border-2 border-[#bfc0c5] p-5 shadow-sm">
            <div className="flex items-center gap-2 mb-3">
              ...
              <p className="text-[15px] font-bold text-[#444]">Export options</p>
            </div>
            <div className="flex flex-col gap-2">
              {[
                { label: 'Export scene configuration', action: exportConfiguration },
                { label: 'Download source image', action: exportImage },
                { label: 'Copy share link', action: shareScene },
              ].map((option) => (
                <button
                  type="button"
                  key={option.label}
